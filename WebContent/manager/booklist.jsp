...
<div class="container">
	...
	<div class="col-xs-12 col-sm-9 col-sm-offset-2">
	   ...
	   <%
		...
		%>
		<p><a href="<%=request.getContextPath()%>/book?bid=<%=book.getBookID()%>"><%=book.getTitle()%></a>
		&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/manager/book?bid=<%=book.getBookID()%>" class="btn btn-success">编辑</a>
		&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/manager/deletebook?bid=<%=book.getBookID()%>" class="btn btn-danger">删除</a>
		</p>
		<%
		}
		%>
	<a href="<%=request.getContextPath()%>/manager/book" class="btn btn-success btn-large">添加图书</a>
	</div>
	</div>
</div>
</div>
