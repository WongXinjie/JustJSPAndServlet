...
<div class="container">
	<div id="content" class="row row-offcanvas row-offcancas-right">
		...
		<%
		List<BookOrder> orderList = (List<BookOrder>)request.getAttribute("orders");
		for(BookOrder order: orderList){
		%>
		<p><%=order.getBookOrderID()%>&nssp;<%=order.getOrderTime()%>&nbsp;<%=order.getAmount()%></p>
		<a href="<%=request.getContextPath()%>/vieworder?caid=<%=order.getBookOrderID()%>" class="btn btn-success">查看订单详情</a>
		<%}%>
		</div>
